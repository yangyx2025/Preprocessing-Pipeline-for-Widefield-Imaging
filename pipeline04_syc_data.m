...
% yyx 20250713 更改数据存储格式 使用字符串替换数字表征状态等
%运行时注意daq通道的对应意义是否一致；
...
FunAddPath()
%% 参数设置
rootpath='K:\m0825\20230905_sleep_sp';%根目录（要求子文件夹命名无误/res /syc/*.conv.tdms )
cam_framerate=10;
...
function sti=FunGetStiTimepointMatrix(syc_event,daq_rate)
    ...
    for i=1:size(sti_edge_timepoint_matrix,1)-1%去掉最后一个trial防止不完整
        trial_time(i,1)=sti_edge_timepoint_matrix(i,1);%trial 开始
        trial_time(i,2)=sti_edge_timepoint_matrix(i,3);%刺激开始
        trial_time(i,3)=sti_edge_timepoint_matrix(i,4);%刺激结束
        trial_time(i,4)=sti_edge_timepoint_matrix(i+1,1)-1;%下一个trial开始前
    end
    %将刺激类型作为数值存储(确定trial type）
    trial_type_number=round(diff(sti_edge_timepoint_matrix(:,1:2),1,2)./100);
    if numel(unique(trial_type_number))>3
        error('trial type 超过三类')
    end
    trial_type_string=categorical(Trans2Column(trial_type_number),[1 2 3],["low" "medium" "high"]);
    %更新时间
    trial_time=(trial_time-syc_event.exp_event(1))./daq_rate;
    %刺激类型对应物理意义
    sti=struct('trial_time',trial_time, ...
        'trial_type',trial_type_string);
end
